...
# Define the conditions for minimal extremism
minimal_conditions = {
    'Ingenuity': 0.0,
    'Utility': 1.0,
    'Impact': 0.0,
    'Ethical Risk': 0.0,
    'Technical Feasibility': 0.0,
    'Security Risk': 0.0,
    'Environmental Impact': 0.0,
    'Public Perception': 1.0
}

# Create a DataFrame with the minimal conditions
df_minimal = pd.DataFrame([minimal_conditions])

# Calculate the minimal extremism score
minimal_extremism_score = calculate_extremism(df_minimal, weights).iloc[0]

print(f"Minimal Extremism Score: {minimal_extremism_score}")
